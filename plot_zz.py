    ...
    for train_subjects, test_subjects in folds:
        ...
        for _ in range(randoms):
            if len(labels[predictor_name]) < 2:
                results['removal_bootstrap'].append('na')
                continue
            ### seven items with replacement
            rand_idxs = random.choices(range(len([labels[predictor_name]])), k=7)
            ...
            results['removal_bootstrap'].extend('+'.join(present_labels)+':{}'.format(boot_corr))
